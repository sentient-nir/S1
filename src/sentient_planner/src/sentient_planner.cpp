...
SentientPlanner::SentientPlanner(const rclcpp::Node::SharedPtr& node, const std::string& group_name)
    : node_(node)
{
    init(group_name);
}

SentientPlanner::SentientPlanner(const std::string& group_name)
{
    node_ = rclcpp::Node::make_shared("sentient_planner_move_group_node");
    init(group_name);
}

void SentientPlanner::init(const std::string& group_name) 
{
...
bool SentientPlanner::planJointTarget(const std::vector<double>& joint_target)
{
    bool success = move_group_->setJointValueTarget(joint_target);
    if (!success)
        RCLCPP_WARN(node_->get_logger(), "setJointValueTarget: out of bounds");
    success = (move_group_->plan(sentient_plan_) == moveit::core::MoveItErrorCode::SUCCESS);
    if (!success)
...
bool SentientPlanner::planPoseTarget(const geometry_msgs::msg::Pose& pose_target)
{
...
bool SentientPlanner::planPoseTargets(const std::vector<geometry_msgs::msg::Pose>& pose_target_vector)
{
...
bool SentientPlanner::planCartesianPath(const std::vector<geometry_msgs::msg::Pose>& pose_target_vector)
{   
    ...
    // sentient_plan_.trajectory_ = trajectory;
    return true;
}

bool SentientPlanner::executePath(bool wait)
{
    moveit::core::MoveItErrorCode code;
    if (wait)
        code = is_trajectory_ ? move_group_->execute(trajectory_) : move_group_->execute(sentient_plan_);
    else
        code =  is_trajectory_ ? move_group_->asyncExecute(trajectory_) : move_group_->asyncExecute(sentient_plan_);
